...
def fitness(path):
    ...
    for i in range(len(path) - 1):
        ...
        if path_length == 0:
            ...
            smooth = sum([d(path[i], path[i + 1], path[i + 2]) for i in range(len(path) - 2)]) / path_length # d calc avg turning angle
...
def collision(start, end):
    for obstacle in obstacle_rects:
        if obstacle.collidepoint(start) or obstacle.collidepoint(end):
            print("Point collided with obstacle")
            return 1
        elif obstacle.clipline(start, end):
            print("Line collided with obstacle")
            return 2
...
def draw_path(path, surface):  # draws path lines along with the coordinatesfor the nodes
    ...
    for i in range(len(path) - 1):
        ...
        if collision(start_point, end_point) != 2 and 1:
            ...
            print("Collision detected, path not drawn")
            l = 1
            l1.append(l)
            obs_detect.append(path)
            print(obs_detect,)
            print(l1)
            break
...
def main():
    ...
    print("\nThe number of mutations = ",sum(mutation_count))
    #print(obs_detect)
    #print(len(obs_detect))
    while True:
